...
class MoodCheckInApp {
    ...
    setupEventListeners() {
        ...
        // Teacher emotional context modal controls
        const closeTeacherEmotionalContextModal = document.getElementById('closeTeacherEmotionalContextModal');
        if (closeTeacherEmotionalContextModal) {
            closeTeacherEmotionalContextModal.addEventListener('click', () => {
                this.hideTeacherEmotionalContextModal();
            });
        }
        ...
        const backToTeacherEmotions = document.getElementById('backToTeacherEmotions');
        if (backToTeacherEmotions) {
            backToTeacherEmotions.addEventListener('click', () => {
                this.showEmotionModal();
            });
        }

        // Teacher emotional context selection
        document.querySelectorAll('.emotional-context-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                this.selectTeacherEmotionalContext(e.target.closest('.emotional-context-btn').dataset.context);
            });
        });
    ...
    // Teacher-specific mood functions
    showTeacherMoodModal() {
        document.getElementById('moodModal').classList.add('active');
        this.selectedMood = null;
        this.selectedTeacherEmotionalContext = null;
        this.updateMoodButtons();
        this.updateTeacherEmotionalContextButtons();
        document.getElementById('confirmMoodCheckin').disabled = true;
        document.getElementById('moodNotes').value = '';
    }

    hideTeacherEmotionalContextModal() {
        document.getElementById('teacherEmotionalContextModal').classList.remove('active');
    }

    selectTeacherEmotionalContext(context) {
        this.selectedTeacherEmotionalContext = context;
        this.updateTeacherEmotionalContextButtons();
        document.getElementById('confirmTeacherMoodCheckin').disabled = false;
    }

    updateTeacherEmotionalContextButtons() {
        document.querySelectorAll('.emotional-context-btn').forEach(btn => {
            btn.classList.remove('selected');
            if (btn.dataset.context === this.selectedTeacherEmotionalContext) {
                btn.classList.add('selected');
            }
        });
    }

    async handleTeacherMoodCheckIn() {
        if (!this.selectedMood || !this.selectedTeacherEmotionalContext || !this.currentUser) return;

        const notes = document.getElementById('teacherMoodNotes').value;
        ...
        try {
            const response = await APIUtils.saveMoodCheckin({
                ...
                notes: notes,
                emotionalContext: this.selectedTeacherEmotionalContext,
                userType: 'teacher'
            });
            ...
            if (response.success) {
                ...
                this.showMessage('Teacher mood check-in saved successfully!', 'success');
                this.updateTeacherStatusDisplay();
                this.updateTeacherAnalytics();
            } else {
                this.showMessage('Failed to save teacher mood check-in: ' + response.error, 'error');
            }
        } catch (error) {
            console.error('Error saving teacher mood check-in:', error);
            this.showMessage('Error saving teacher mood check-in. Please try again.', 'error');
        }
    }
    ...
    updateTeacherStatusDisplay() {
        ...
        const lastMood = this.moodHistory[0];
        if (lastMood) {
            const timeAgo = this.getTimeAgo(lastMood.timestamp);
            teacherStatusDisplay.innerHTML = `
                <p class="status-message">
                    Last check-in: <strong>${lastMood.emoji} ${lastMood.mood}</strong> - ${timeAgo}
                </p>
            `;
        } else {
            teacherStatusDisplay.innerHTML = `
                <p class="status-message">Ready for your daily check-in!</p>
            `;
        }
    }

    // Emotion modal functions
    showEmotionModal() {
        document.getElementById('emotionModal').classList.add('active');
        this.selectedEmotions = [];
        this.updateEmotionButtons();
        document.getElementById('proceedToLocation').disabled = true;
    }
    ...
    selectEmotion(emotion) {
        ...
        const proceedButton = document.getElementById('proceedToLocation');
        if (proceedButton) {
            proceedButton.disabled = this.selectedEmotions.length === 0;
        }
    }
    ...
    // Teacher-specific flow: proceed from emotions to emotional context instead of location
    proceedFromEmotions() {
        if (this.currentUser && this.currentUser.user_type === 'teacher') {
            this.hideEmotionModal();
            this.showTeacherEmotionalContextModal();
        } else {
            this.hideEmotionModal();
    ...
    showTeacherEmotionalContextModal() {
        document.getElementById('teacherEmotionalContextModal').classList.add('active');
        this.selectedTeacherEmotionalContext = null;
        this.updateTeacherEmotionalContextButtons();
        document.getElementById('confirmTeacherMoodCheckin').disabled = true;
        document.getElementById('teacherMoodNotes').value = '';
    }
