...
class MoodCheckInApp {
    constructor() {
        ...
        this.unlockedTileQuotes = [];
        this.currentCollectedQuoteIndex = 0;
    ...
    async initializeApp(retryCount = 0) {
        ...
        // Restore session from server (cookie-based auth)
        try {
            const meResponse = await APIUtils.getMe();
            if (meResponse.success && meResponse.user) {
                this.currentUser = meResponse.user;
    ...
    applyTileFlipVisibility() {
        ...
        if (tileFlipCard) tileFlipCard.style.display = enabled ? '' : 'none';
        if (quotesCard) quotesCard.style.display = enabled ? '' : 'none';
    }
    ...
    setupStudentDashboardTabs() {
        ...
        this.setupQuoteCollectionArrows();
    }
    ...
    async initializeTileFlip() {
        ...
        try {
            ...
            if (statusResponse.success) {
                ...
                this.unlockedTileQuotes = statusResponse.unlockedQuotes || [];
                this.currentCollectedQuoteIndex = Math.min(this.currentCollectedQuoteIndex, Math.max(0, this.unlockedTileQuotes.length - 1));
            }
    ...
    async handleTileFlip(tileIndex) {
        ...
        try {
            ...
            if (response.success) {
                ...
                this.unlockedTileQuotes = response.unlockedQuotes || [];
                this.currentCollectedQuoteIndex = Math.max(0, this.unlockedTileQuotes.length - 1);
    ...
    renderQuoteCollection() {
        ...
        const emptyEl = document.getElementById('quoteCollectionEmpty');
        const contentEl = document.getElementById('quoteCollectionContent');
        const textEl = document.getElementById('quoteDisplayText');
        const authorEl = document.getElementById('quoteDisplayAuthor');
        ...
        const quotes = this.unlockedTileQuotes || [];
        const idx = Math.max(0, Math.min(this.currentCollectedQuoteIndex, quotes.length - 1));
        this.currentCollectedQuoteIndex = quotes.length ? idx : 0;

        if (quotes.length === 0) {
            emptyEl.classList.remove('student-tab-hidden');
            contentEl.classList.add('student-tab-hidden');
            return;
        }

        emptyEl.classList.add('student-tab-hidden');
        contentEl.classList.remove('student-tab-hidden');

        const q = quotes[this.currentCollectedQuoteIndex];
        if (textEl) textEl.textContent = q.text ? `"${q.text}"` : '';
        if (authorEl) authorEl.textContent = q.author ? `— ${q.author}` : '';
        if (counterEl) counterEl.textContent = `${this.currentCollectedQuoteIndex + 1} / ${quotes.length}`;

        if (prevBtn) {
            prevBtn.disabled = quotes.length <= 1 || this.currentCollectedQuoteIndex <= 0;
        }
        if (nextBtn) {
            nextBtn.disabled = quotes.length <= 1 || this.currentCollectedQuoteIndex >= quotes.length - 1;
        }
    }

    setupQuoteCollectionArrows() {
        const prevBtn = document.getElementById('quoteNavPrev');
        const nextBtn = document.getElementById('quoteNavNext');
        ...
        prevBtn.addEventListener('click', () => {
            ...
            this.currentCollectedQuoteIndex = Math.max(0, this.currentCollectedQuoteIndex - 1);
            this.renderQuoteCollection();
        });
        nextBtn.addEventListener('click', () => {
            const quotes = this.unlockedTileQuotes || [];
            if (quotes.length === 0) return;
            this.currentCollectedQuoteIndex = Math.min(quotes.length - 1, this.currentCollectedQuoteIndex + 1);
            this.renderQuoteCollection();
        });
    }
        ...
        try {
            ...
            if (statusResponse.success && statusResponse.shouldReset) {
                ...
                if (newStatus.success) {
                    ...
                    this.unlockedTileQuotes = newStatus.unlockedQuotes || [];
                    this.currentCollectedQuoteIndex = 0;
                    this.renderQuoteCollection();
                }
            }
...
// Teacher Class Management
MoodCheckInApp.prototype.updateTeacherClassDisplay = async function () {
...
MoodCheckInApp.prototype.openTeacherClassModal = async function () {
    if (!this.currentUser || this.currentUser.user_type !== 'teacher') return;
...
MoodCheckInApp.prototype.closeTeacherClassModal = function () {
    const modal = document.getElementById('teacherClassModal');
    if (modal) {
        modal.style.display = 'none';
        modal.classList.remove('active');
...
MoodCheckInApp.prototype.saveTeacherClass = async function () {
    if (!this.currentUser || this.currentUser.user_type !== 'teacher') return;
...
MoodCheckInApp.prototype.loadTeacherClassCheckins = async function (period = 'daily') {
    if (!this.currentUser || this.currentUser.user_type !== 'teacher') return;
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        // Guard against double initialisation (e.g., if multiple entrypoints load).
        if (window.moodApp) {
            appDebugLog('MoodCheckInApp already initialised; skipping duplicate init in root app-api.js');
            return;
        }
